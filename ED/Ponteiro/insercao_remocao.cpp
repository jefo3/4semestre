...
Pseudocódigo:
   altere o tamanho do vetor v para conter *n+1 e pondo endlementos,ereço em v
...
aluno* insere_aluno(aluno *v, int *n, aluno novo){
   ...
   if(*n == 0){
      ...
      v = (aluno*) realloc(v, sizeof(aluno) * (*n+1));
   }

   v[*n] = novo;
    //cout << *n+1 << " " << v[*n].matricula; 
   *n = *n + 1;
   return v;
}
...
aluno* remove_aluno(aluno *v, int *n, int matricula){
   ...
   *n = *n - 1;
   v = (aluno*) realloc(v, sizeof(aluno) * (*n));
   return v;
}
